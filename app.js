...
// veryOldInventors = inventors.filter((years) =>{
//     years = (inventors.year >= 1500 && inventors.year <1600)
//     return years;
// }
// )
...
inventorNames = inventors.map((invName)=>{
    // return "first : " + invName.first + "    last : " + invName.last;
    return `first: ${invName.first} , last: ${invName.last}`;
})
// Check your work:
...
// Complete the exercise in the space below:
inventorNamedAda = inventors.find((fixName) => {
    return fixName.first === "Ada";
});
// Check your work:
...
isAdultPresent = devs.some((ages) => {
    // console.log("test", ages.year<= 2006);              // Testing if it works.
    // return ages.year > 2016;                            // Testing if it works.
    return ages.year <= (2024-18);
});
// Check your work:
...
// Complete the exercise in the space below:
isEveryone19OrOlder = devs.every((ageChecker)=>{
    return ageChecker.year < (2024-19); 
});
...
// Complete the exercise in the space below:
commentById = comments.find((commId)=>{
    return commId.id === 823423;
});
...
idx = comments.findIndex((index)=>{
    // return index.id === 542328;                  //Testing if it works.
